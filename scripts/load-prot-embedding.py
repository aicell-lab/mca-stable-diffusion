...
A script for loading protein embedding

The downlaoded and saved to "./data/per-protein.h5"

Here are the documentation from uniprot:
...
UniProt is providing raw embeddings (per-protein and per-residue using the ProtT5 model) for UniProtKB/Swiss-Prot and some reference proteomes of model organisms (such as Homo sapiens, SARS-CoV-2, and E. coli). You can retrieve them from our Downloads page.
...
Sample code
The embeddings.h5 files store the embeddings as key-value pairs. The key is the protein accession number and the value is the embeddings vector. The following code snippet shows how to read and iterate over an embeddings file in python.
...
# load the file and count the number of entries
